...
R=Path(__file__).resolve().parents[2]; B=R/'telegram_avatars/concept02'
...
for slug,title,desc,bg,kind in items:
 ...
 s=f'<svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1024" viewBox="0 0 1024 1024"><rect width="1024" height="1024" fill="{bg}"/><circle cx="512" cy="512" r="460" fill="none" stroke="{fg}" stroke-width="6" opacity=".13"/>'
 if kind=='news':
 ...
 # Deliberately secondary branding, common to all roles. No tiny role badges.
 wordw=ns['tw']('MOKO',70,'Jost',500);total=82+25+wordw;x=(1024-total)/2
 s+=mark(x,794,82,mono=fg,opacity=.76)+text('MOKO',x+107,850,70,fg,'Jost',500,opacity=.76)+'</svg>'
...
d.text((70,48),'MOKO / TELEGRAM · КОНЦЕПТ 02',font=f(25),fill='#A4B4C9')
d.text((70,103),'Сначала назначение. Потом бренд.',font=f(43),fill=W)
d.text((70,172),'Крупный символ + отдельный цвет всего аватара',font=f(24),fill='#A4B4C9')
...
d.text((912,1308),'Символы теперь главные.\nMOKO — небольшая подпись.',font=f(21),fill='#A4B4C9',spacing=10)
board.save(B/'MOKO_Telegram_Концепт_02.jpg',quality=94,optimize=True)
...
s='<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>MOKO Telegram — концепт 02</title><style>'+css+'</style></head><body><main><div class="eyebrow">MOKO / TELEGRAM · КОНЦЕПТ 02</div><h1>Сначала назначение.<br>Потом бренд.</h1><p class="intro">Вместо шести одинаковых больших знаков MOKO — шесть крупных символов роли. Цвет занимает весь фон. Фирменная подпись остаётся, но больше не мешает различать аватары.</p><p class="note">Нажмите на аватар, чтобы увеличить. Картинки показаны в круглой обрезке; файлы PNG для установки остаются квадратными.</p><div class="grid">'
...
s+='</div><h2>Проверка в списке чатов · 48 px</h2><p>Условный макет, не снимок вашего Telegram.</p><div class="list">'
for r in records:s+=f'<div class="row"><img src="{uri(r)}" alt=""><div><strong>{r["title"]}</strong><p>{r["description"]}</p></div></div>'
s+='</div><h2>Ещё меньше · 40 px</h2><div class="small">'+''.join(f'<img src="{uri(r)}" alt="{r["title"]}">' for r in records)+'</div><p class="note">Голубая газета — новости. Фиолетовые диалоги — чат. Красная сетка — группа с разделами. Бирюзовый робот SE — диспетчер. Оранжевая гарнитура — связь с командой. Тёмный фон с золотистыми JR — Johnny Respect.</p><h2>Что в комплекте</h2><p>Шесть PNG 1024 × 1024, шесть SVG в контурах, общий обзор и эта автономная презентация. MOKO использован как вторичная фирменная подпись, в одноцветной версии. Гарнитура обозначает связь, а не наличие функции звонков.</p><p>Названия — подписи для выбора файла, а не требование переименовать паблики. Концепт 01 сохранён отдельно. Ничего не опубликовано.</p><footer class="foot">Концепт 02 · На согласование · 09.09.2026</footer></main><dialog id="zoom"><div class="bar"><span id="caption"></span><button id="close">Закрыть ✕</button></div><img id="large" alt=""></dialog><script>const d=document.getElementById("zoom"),im=document.getElementById("large");document.querySelectorAll(".open").forEach(b=>b.onclick=()=>{const s=b.querySelector("img");im.src=s.src;im.alt=s.alt;document.getElementById("caption").textContent=s.alt;d.showModal()});document.getElementById("close").onclick=()=>d.close();d.onclick=e=>{if(e.target===d)d.close()};</script></body></html>'
(B/'MOKO_Telegram_Концепт_02.html').write_text(s)
readme='MOKO / TELEGRAM — КОНЦЕПТ 02\nНа согласование\n\nГлавное отличие: назначение аватара крупнее бренда. Цвет занимает весь фон. Небольшая подпись MOKO объединяет серию.\n\n'
...
q={'concept':2,'status':'awaiting_approval','published':False,'files':[]}
for r in records:
...
with zipfile.ZipFile(B/'MOKO_Telegram_Концепт_02.zip','w',zipfile.ZIP_DEFLATED,compresslevel=9) as z:
 for sub in ['png','svg']:
  for p in sorted((B/sub).iterdir()):z.write(p,p.relative_to(B))
 for name in ['MOKO_Telegram_Концепт_02.html','MOKO_Telegram_Концепт_02.jpg','README.txt']:z.write(B/name,name)
with zipfile.ZipFile(B/'MOKO_Telegram_Концепт_02.zip') as z:assert z.testzip() is None
print('Concept 02 ready:',(B/'MOKO_Telegram_Концепт_02.zip').stat().st_size,'bytes')
